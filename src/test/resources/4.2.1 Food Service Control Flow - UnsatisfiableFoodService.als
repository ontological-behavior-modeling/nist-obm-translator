//*****************************************************************
// Module: 		Food Service Control Flow - Usatisfiable Food Service
...
//*****************************************************************
module UnsatisfiableFoodServiceModule
...
fact {all x: UnsatisfiableFoodService | bijectionFiltered[happensBefore, x.eat, x.pay]}
fact {all x: UnsatisfiableFoodService | bijectionFiltered[happensBefore, x.pay, x.prepare]}
//	have to loosen the symmetry constraint in the library module to produce an instance.
/** Model closure */
fact {all x: UnsatisfiableFoodService | x.steps in x.eat + x.order + x.pay + x.prepare + x.serve}
fact {all x: UnsatisfiableFoodService | no x.inputs}
fact {all x: UnsatisfiableFoodService | no inputs.x}
fact {all x: UnsatisfiableFoodService | no x.outputs}
fact {all x: UnsatisfiableFoodService | no outputs.x}
fact {all x: UnsatisfiableFoodService | no items.x}
fact {all x: UnsatisfiableFoodService | no steps.x}
fact {all x: UnsatisfiableFoodService | no y: Transfer | y in x.steps}
//******************************************************************************************************
/** 			General Functions and Predicates */
//******************************************************************************************************
pred instancesDuringExample{all x: (Eat + Order + Pay + Prepare + Serve) | x in UnsatisfiableFoodService.steps}
//******************************************************************************************************
/** 				Checks and Runs */
...
//******************************************************************************************************
run showRestaurantService{instancesDuringExample and some UnsatisfiableFoodService and FoodService in UnsatisfiableFoodService and no BinaryLink} for 10
