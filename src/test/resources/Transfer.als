...
fact transfersOccurDuringTheirItems{all t: Transfer | all item: t.items | during[t, item]}
...
fact transferEachItemFromSource{all t: Transfer | t.items in (t.sources.outputs + t.sources.inputs)}
fact transferEachItemFromSource{all t: Transfer | all i: t.items | i in (t.sources.outputs + t.sources.inputs)}
fact transferEachItemToTarget {all t: Transfer | t.items in (t.targets.inputs + t.targets.outputs)}
fact transferEachItemToTarget {all t: Transfer | all i: t.items | i in (t.targets.inputs + t.targets.outputs)}
fact disableTransferIO {noTransferInputsOrOutputs[Transfer]}
...
pred uniqueItems [transfers: set Transfer] {all disj t, t': transfers | all i: t.items | all i': t'.items | i not in t'.items and i' not in t.items}
...
pred transferEveryItemFromOutputOfSource [transfers: set Transfer] {
	all t: transfers | all src: t.sources | all occ: Occurrence | occ in src.outputs <=> occ.inputs in t.items || occ.outputs in t.items}
...
// Remove and replace on 2023-11-21
...
fact occurrenceHappensDuringInput {all o: Occurrence | all input: o.inputs | during[o, input]}
fact occurrenceHappensDuringOutput {all o: Occurrence | all output: o.outputs | during[o, output]}
//*************************** end
...
// These two are main. Next two added for UnsatisfiableTransitivity. Swap commented lines as needed.
fact beforeIsTransitive{all x,y,z: Occurrence | before[x,y] and before[y,z] => before[x,z]}
fact beforeIsAsymmetric{all x: Occurrence | not before[x,x]}
//fact beforeIsTransitive{r/transitive[happensBefore]}
//fact beforeIsAsymmetric{r/asymmetric[happensBefore]}

fact duringIsReflexive{all x: Occurrence | during[x,x]}
fact duringIsTransitive{all x,y,z: Occurrence | during[x,y] and during[y,z] => during[x,z]}
/** Should probably remove this beforeSymmetry constraint at this point */
...
fact stepImpliesInverseDuring {all x,y: Occurrence | y in x.steps => during[y,x]}
...
/** Proposed constraints to add for inputs, outputs, and possibly items */
//fact cannotHappenBeforeOrAfterOutput {all x, y: Occurrence | y in x.outputs => not before[x,y] and not before[y,x]}
//fact cannotHappenBeforeOrAfterInput {all x, y: Occurrence | y in x.inputs => not before[x,y] and not before[y,x]}
