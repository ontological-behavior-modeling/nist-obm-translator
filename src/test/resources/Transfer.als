...
one sig OccurrenceSig {
	HappensDuring, HappensBefore: Occurrence -> Occurrence,	
...
fact occurrenceHappensDuringInputEqualsOutput {
	all o: Occurrence | all input: o.inputs | all output: o.outputs | input = output => during[o, input]
}
...
fact interrelationshipConstraint2{all x,y,z: Occurrence | before[y,x] and during[z,y] => before[z,x]}
...
// Added per NIST PPT
check duringAZeroDurationImpliesZeroDuration {all x,y: Occurrence | before[x,x] and during[y,x] => before[y,y] and before[y,x]} for 20					// 1.a
check beforeEachOtherImpliesDuringEachOtherAndZeroDuration {all x,y: Occurrence | before[x,y] and before[y,x] => during[x,y] and before[x,x]} for 20		// 1.b
check beforeEachOtherImpliesDuringEachOtherAndZeroDuration' {all x,y: Occurrence | before[x,y] and before[y,x] => during[y,x] and before[y,y]} for 20 		// 1.b'
check afterAndDuringImpliesZeroDuration {all x,y: Occurrence | before[x,y] and during[y,x] => before[y,y]} for 20										// 1.c
