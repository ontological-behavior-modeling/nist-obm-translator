...
class SimpleSequenceImplicitFactTest {
	...
	void test() {
	    ...
	    Expr functionFilteredExpression = functionFilteredFunction.call(happensBefore.call(), thisVar.join(p1), thisVar.join(p2));    
	    Expr inverseFunctionFilteredExpression = inverseFunctionFilteredFunction.call(happensBefore.call(), thisVar.join(p1), thisVar.join(p2));
	    ...
	    Func p1DuringExamplePredicate = new Func(null, "p1DuringExample", new ArrayList<>(), null, p1Sig.in(mainSig.join(p1)));
	    Expr p1DuringExampleExpression = p1DuringExamplePredicate.call();
	    
	    Func p2DuringExamplePredicate = new Func(null, "p2DuringExample", new ArrayList<>(), null, p2Sig.in(mainSig.join(p2)));
	    Expr p2DuringExampleExpression = p2DuringExamplePredicate.call();
	    
	    Func instancesDuringExamplePredicate = new Func(
    		null, "instancesDuringExample", new ArrayList<>(), null, p1DuringExampleExpression.and(p2DuringExampleExpression));
	    Expr instancesDuringExampleExpression = instancesDuringExamplePredicate.call();
	    		
	    Func onlySimpleSequencePredicate = new Func(
    		null, "onlySimpleSequence", new ArrayList<>(), null, mainSig.cardinality().equal(ExprConstant.makeNUMBER(1)));
	    Expr onlySimpleSequenceExpression = onlySimpleSequencePredicate.call();
	    
	    Expr predicates = nonZeroDurationOnlyFunctionExpression.and(suppressTransfersExpression).and(suppressIOExpression).and(instancesDuringExampleExpression).and(onlySimpleSequenceExpression);
