...
class CallingBehaviorsTest {
  ...
  void test() {
    ...
    nestedBehaviorSig.addFact(bijectionFilteredFunction
        .call(happensBeforeFunction.call(), this_nestedBehavior.join(nestedBehavior_p4),
            this_nestedBehavior.join(nestedBehavior_p5))
        .and(this_nestedBehavior.join(nestedBehavior_p4).cardinality()
            .equal(ExprConstant.makeNUMBER(1)))
        .and(this_nestedBehavior.join(nestedBehavior_p4)
            .plus(this_nestedBehavior.join(nestedBehavior_p5))
            .in(this_nestedBehavior.join(stepsFunction.call())))
        .and(this_nestedBehavior.join(stepsFunction.call()).in(this_nestedBehavior
            .join(nestedBehavior_p4).plus(this_nestedBehavior.join(nestedBehavior_p5)))));

    // ComposedBehavior

    ExprVar this_composedBehavior = ExprVar.make(null, "this", composedBehaviorSig.type());

    composedBehaviorSig.addFact(bijectionFilteredFunction
        .call(happensBeforeFunction.call(), this_composedBehavior.join(composedBehavior_p1),
            this_composedBehavior.join(composedBehavior_p2))
        .and(bijectionFilteredFunction.call(happensBeforeFunction.call(),
            this_composedBehavior.join(composedBehavior_p2),
            this_composedBehavior.join(composedBehavior_p3)))
        .and(this_composedBehavior.join(composedBehavior_p1).cardinality()
            .equal(ExprConstant.makeNUMBER(1)))
        .and(this_composedBehavior.join(composedBehavior_p1)
            .plus(this_composedBehavior.join(composedBehavior_p2))
            .plus(this_composedBehavior.join(composedBehavior_p3))
            .in(this_composedBehavior.join(stepsFunction.call())))
        .and(this_composedBehavior.join(stepsFunction.call())
            .in(this_composedBehavior.join(composedBehavior_p1)
                .plus(this_composedBehavior.join(composedBehavior_p2))
                .plus(this_composedBehavior.join(composedBehavior_p3)))));
    ...
    Expr composedBehaviorCmdExpr =
        nonZeroDurationOnlyExpression.and(suppressTransfersExpression).and(suppressIOExpression)
