...
class LoopsExample_ExplicitFact_Test {
	...
	void test() {
		...
		alloy.createInverseFunctionFilteredHappensBeforeAndAddToOverallFact(
		loopSig, loop_p2, loop_p3);
		
		// #s.p1 = 1
		
		ExprVar s = ExprVar.make(null, "s", loopSig.type());
		Decl decl = new Decl(null, null, null, List.of(s), loopSig.oneOf());
		
		alloy.addToOverallFact(s.join(loop_p1).cardinality()
		.equal(ExprConstant.makeNUMBER(1)).forAll(decl));
		
		// #s.p2 >= 2
		ExprVar s2 = ExprVar.make(null,  "s", loopSig.type());
		Decl decl2 = new Decl(null, null, null, List.of(s2), loopSig.oneOf());
		
		alloy.addToOverallFact(s2.join(loop_p2).cardinality()
		.gte(ExprConstant.makeNUMBER(2)).forAll(decl2));
		
		// #s.p3 >= 1
		ExprVar s3 = ExprVar.make(null,  "s", loopSig.type());
		Decl decl3 = new Decl(null, null, null, List.of(s3), loopSig.oneOf());
		
		alloy.addToOverallFact(s3.join(loop_p3).cardinality()
		.gte(ExprConstant.makeNUMBER(1)).forAll(decl3));
		
		// s.p1 + s.p2 + s.p3 in s.steps
		ExprVar s4 = ExprVar.make(null, "s", loopSig.type());
		Decl decl4 = new Decl(null, null, null, List.of(s4), loopSig.oneOf());
		
		Func stepFunctions = Helper.getFunction(
		Alloy.transferModule, "o/steps");
		
		alloy.addToOverallFact(s4.join(loop_p1).plus(s4.join(loop_p2))
		.plus(s4.join(loop_p3)).in(s4.join(stepFunctions.call()))
		.forAll(decl4));
		...
		String filename = 
		"src/test/resources/4.1.2 LoopsExample-Explicit-Facts.als";
	    ...
	    // p1DuringExample
	    Expr p1DuringExampleExpr = p1Sig.in(loopSig.join(loop_p1));
	    Func p1DuringExampleFunc = new Func(null, "p1DuringExample", 
		List.of(), null, p1DuringExampleExpr);
	    
	    // p2DuringExample
	    Expr p2DuringExampleExpr = p2Sig.in(loopSig.join(loop_p2));
	    Func p2DuringExampleFunc = new Func(null, "p2DuringExample",
		List.of(), null, p2DuringExampleExpr);
	    
	    // p3DuringExample
	    Expr p3DuringExampleExpr = p3Sig.in(loopSig.join(loop_p3));
	    Func p3DuringExampleFunc = new Func(null, "p3DuringExample",
		List.of(), null, p3DuringExampleExpr);
	    
	    // p4DuringExample
	    Expr p4DuringExampleExpr = p4Sig.no();
	    Func p4DuringExampleFunc = new Func(null, "p4DuringExample", 
		List.of(), null, p4DuringExampleExpr);
	    
	    // instancesDuringExample
	    Expr instancesDuringExampleExpr = p1DuringExampleExpr
		.and(p2DuringExampleExpr).and(p3DuringExampleExpr)
		.and(p4DuringExampleExpr);
	    Func instancesDuringExampleFunc = new Func(null,
		"instancesDuringExample", List.of(), null, 
		instancesDuringExampleExpr);
	    
	    // onlyLoop
	    Expr onlyLoopExpr = loopSig.cardinality()
		.equal(ExprConstant.makeNUMBER(1));
	    Func onlyLoopFunc = new Func(null, "onlyLoop", List.of(), null, 
		onlyLoopExpr);
		...
		.and(instancesDuringExampleFunc.call()).and(onlyLoopFunc.call());
	    Command loopCmd = new Command(null, loopExpr, "loop", false, 12, -1, 
		-1, -1, List.of(), List.of(), loopExpr.and(alloy.getOverAllFact()), 
		null);
	    
	    Command[] commands = new Command[] {loopCmd};
	    
	    // ========== Write file ==========
	    
	    AlloyModule alloyModule = new AlloyModule(
		"LoopsExample_ExplicitFact", alloy.getAllSigs(),
