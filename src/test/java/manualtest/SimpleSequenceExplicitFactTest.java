...
class SimpleSequenceExplicitFactTest {
  ...
  void test() {
    ...
    Expr simpleSequenceExplicitFactExpr = nonZeroDurationOnlyFunctionExpression
        .and(suppressTransfersExpression).and(suppressIOExpression)
        .and(instancesDuringExampleExpression).and(onlySimpleSequenceExpression);
    Command simpleSequenceExplicitFactCmd =
        new Command(null, simpleSequenceExplicitFactExpr, "SimpleSequence", false, 6, -1, -1, -1,
    ...
    AlloyModule alloyModule = new AlloyModule("SimpleSequence_ExplicitFact", alloy.getAllSigs(),
