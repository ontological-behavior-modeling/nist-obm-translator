...
public class AlloyExprFactory {
   ...
   * @return (Set<Expr>) - one or two expression ([bijection], [bijection, isBeforetarget] or [bijection, isAfterSource])
   */
  protected static Set<Expr> exprs_bijectionFilteredFactsForSig(Sig _ownerSig, Expr _from, Expr _to,
      Func _func) {
   ...
   * 
   */
  protected static Expr expr_functionFilteredHappensBeforeForSig(Sig _ownerSig, Expr _from,
      Expr _to) {
    ExprVar varX = makeVarX(_ownerSig);
    Expr funcFilteredExpr = Alloy.functionFiltered.call(Alloy.happensBefore.call(),
        AlloyUtils.addExprVarToExpr(_from, varX), AlloyUtils.addExprVarToExpr(_to, varX));
   ...
   */
  protected static Expr expr_inverseFunctionFilteredHappensBefore(Sig _ownerSig, Expr _from,
      Expr _to) {
    ExprVar varX = makeVarX(_ownerSig);
  ...
  protected static Expr exprs_subSettingItemRule(Sig _ownerSig, Expr _transferExpr) {
    ...
    return Alloy.subsettingItemRuleForSources.call(varX.join(_transferExpr)).forAll(decl)
   ...
   * @param _transferExpr (Expr) - the transfer expression
   * @return (Set<Expr>) - the isAfterSource and isBeforeTarget Expressions for the given transfer expression for the given signature
   */
  ...
  /**
   * Adds the cardinality equal constraint to field (i.e, fact {all x: ParameterBehavior | #(x.a) = 1})
   *
   * @param _ownerSig (Sig) - the owner signature
  ...
  protected static Expr expr_cardinalityEqual(Sig _ownerSig, Sig.Field _field, int _number) {
    ExprVar varX = makeVarX(_ownerSig);
    return varX.join(_field).cardinality().equal(ExprConstant.makeNUMBER(_number))
   ...
   * @return (Expr) - the created expression
   */
  protected static Expr expr_cardinalityGreaterThanEqual(Sig _ownerSig, Sig.Field _field,
      int _number) {
    ExprVar varX = makeVarX(_ownerSig);
    return varX.join(_field).cardinality().gte(ExprConstant.makeNUMBER(_number))
        .forAll(makeDecl(_ownerSig, varX));
  }
  ...
  /**
   * Create two expressions like {all x: B1 | x.vin in x.inputs} and its closure like {all x: B1 | x.inputs in x.vin}.
   * 
   * The other example {all x: OFCustomPrepare | x.prepareDestination + x.preparedFoodItem in x.inputs} and its closure like {all x: OFCustomPrepare | x.inputs in x.prepareDestination +
   * x.preparedFoodItem}
   * 
   * @param _ownerSig (PrimSig) - the owner signature
   * @param _sortedFields (List<Field>) - the fields of the owner signature alphabetically sorted by their labels
   * @param _inOrOutFunc (Func) - Alloy.oinputs or Alloy.ooutput function
   * @return (Set<Expr>) - the created two expressions
   */
  protected static Set<Expr> exprs_in(PrimSig _ownerSig, List<Sig.Field> _sortedFields,
      Func _inOrOutFunc) {
    ExprVar varX = makeVarX(_ownerSig);
    Decl decl = makeDecl(_ownerSig, varX);
  ...
  /**
   * Create a closure expression like {all x: OFSingleFoodService | x.prepare.inputs in x.prepare.preparedFoodItem + x.prepare.prepareDestination} where _ownerSig = OFSingleFoodService, _fieldOwner =
   * prepare, _fieldsOfField = [preparedFoodItem. prepareDestination], _inOrOutFunc = inputs.
   * 
   * @param _ownerSig (PrimSig) - the owner signature
   * @param _fieldOwner (Field) - a field (a relation to the owner signature) for the owner signature
   * @param _fieldsOfField (List<Field>) - a field type signature fields (i.e., prepareFoodItem and prepareDestination are for prepare field type of Prepare signature).
   * @param _inOrOutFunc (Func) - Alloy.oinputs or Alloy.ooutputs function
   * @return (Expr) - the created expression
   */
  protected static Expr expr_inOutClosure(PrimSig _ownerSig, Field _fieldOwner,
      List<Sig.Field> _fieldsOfField, Func _inOrOutFunc) {
    ExprVar varX = makeVarX(_ownerSig);
    Expr fieldsExpr = null;
    for (Field field : _fieldsOfField)
      fieldsExpr = fieldsExpr == null ? varX.join(_fieldOwner).join(field)
  ...
  /**
   * Create no inputs expression and its closure expression connected with and "{all x| no inputs.x} and {all x| no x.inputs}" and return
   * 
   * @param _sig (Sig) - the owner signature of the creating expression
  ...
  /**
   * Create no outputs expression and its closure expression connected with and "{all x| no x.outputs} and {all x| no outputs.x}" and return
   * 
   * @param _sig (Sig) - the owner signature of the creating expression
  ...
  /**
   * Create one no expression and its closure expression connected with and (ie., "{all x| no inputs.x} and {all x| no x.inputs}") and return
   * 
   * @param _sig (Sig) - the owner signature of the creating expression
   * @param _func (Func) - function for the expression
   * @return (Expr) - the two created expression connected with and
  ...
  protected static Expr exprs_noFuncXAndnoXFunc(Sig _sig, Func _func) {
    ...
    return ((_func.call().join(var).no()).forAll(decl).and(
        (var.join(_func.call()).no()).forAll(decl)));
  }
  ...
  /**
   * Create an expression for a field to be no inputs or outputs like {all x: TheGivenSig | no x.p4.inputs} or {all x: TheGivenSig |no x.p4.outputs} and return
   * 
   * @param _sig (Sig) - the owner signature of the creating expression
   ...
   * @param _fieldOfsigFieldType (Field) - a field (i.e., i) of sig field(i.e, p1)'s type (BehaviorWithParameter)
   ...
   */
  protected static Expr exprs_inField(Sig _sig, Sig.Field _sigField,
      Sig.Field _fieldOfsigFieldType, Func _func) {
  ...
  /**
   * Create step expressions { {... in x.steps} for all signatures and {x.steps in ...} or {no steps.x} for leaf signatures and return
   * 
   * <pre>
   * For example, 
   ...
   * 
   * @param _sig(Sig) - the owner signature of the creating expression
   * @param _stepFields (Set<String>) - field names of the signature to be included in step expression
   * @param _addInXSteps (boolean) - if true, create a expression for leaf or non-leaf signature like {.... in x.steps}
   * @param _addXStepsIn (boolean) - if true, create a expression for leaf-signature to create {x.steps in ....} or {no steps.x}
   * @return (Set<Expr>) - the created expressions
   */
  protected static Set<Expr> exprs_stepsFields(PrimSig _sig, Set<String> _stepFields,
      boolean _addInXSteps, boolean _addXStepsIn) {
    ExprVar varX = makeVarX(_sig);
    ...
    if (_addInXSteps) {// for all sigs - own fields - not include redefined
      ...
      else if (_addXStepsIn) // if leaf sig, then addXStepsIn = true -> if sig has no own fields then
                             // add {no steps.x}
  ...
  /**
   * Create a expression like {all x: OFControlLoopFoodService | x.prepare in OFCustomPrepare} and return The expression is to redefine field type specified by parent signature (FoodService.prepare:
   * Prepare > OFFoodService.prepare: OFPrepare > OFControlLoopFoodService.prepare: OFCustomPrepare) by a constraint
   * 
   * @param _sig (Sig) - the owner signature of the creating expression (i.e., OFControlLoopFoodService)
  ...
  /**
   * Create Decl for signature with the given expression variable as "one of" (i.e, x: TheGivenSig) and return. Combine with for all and used as a part of expression (i.e., "x: OFFoodService" of "{all
   * x: OFFoodService | x.eat in OFEat }")
   * 
   * @param _sig (Sig) - the owner signature of the creating expression (i.e., OFControlLoopFoodService)
  ...
  /**
   * Create Decl for signature with Decl x as "one of" and return. Combine with for all and used as a part of expression (i.e., "x: OFFoodService" of "{all x: OFFoodService | x.eat in OFEat }"}).
   * 
   * @param _sig (Sig) - the owner signature of the creating expression (i.e., OFControlLoopFoodService)
   * @return (Decl) the created Decl
   */
