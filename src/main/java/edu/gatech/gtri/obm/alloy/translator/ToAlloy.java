...
public class ToAlloy {
  ...
  protected PrimSig createSigOrReturnSig(String _name, String _parentName) {
    ...
    if (!AlloyUtils.isValidUserDefineParent(_parentName)) // null, Occurrence, BehaviorOccurrence or Anything
   ...
   */
  protected Sig.Field[] addDisjAlloyFields(List<String> _fieldNamesListWithSameType,
      String _fieldSigTypeName, PrimSig _ownerSig) {
   ...
   */
  protected void createInFieldExpression(Sig _ownerSig, Sig.Field _sigField,
      Sig.Field _fieldOfsigFieldType, Func _inputsOrOutputs) {
    alloy.addToFacts(AlloyExprFactory.exprs_inField(_ownerSig, _sigField, _fieldOfsigFieldType,
        _inputsOrOutputs));
  }
  ...
  protected void addBijectionFilteredHappensBeforeFact(Sig _ownerSig, Expr _from, Expr _to) {
    alloy.addToFacts(
        AlloyExprFactory.exprs_bijectionFilteredFactsForSig(_ownerSig, _from, _to,
            Alloy.happensBefore));
  }
   ...
   */
  protected void addFunctionFilteredHappensBeforeFact(Sig _ownerSig, Expr _from,
      Expr _to) {
    alloy.addToFacts(
   ...
   */
  protected void addInverseFunctionFilteredHappensBeforeFact(Sig _ownerSig,
      Expr _from, Expr _to) {
    alloy.addToFacts(
  ...
  protected void addCardinalityFact(PrimSig _ownerSig, Field _field, int _lower, int _upper) {
    if (_lower == _upper)
      ...
      alloy
          .addToFacts(AlloyExprFactory.expr_cardinalityGreaterThanEqual(_ownerSig, _field, _lower));
    }
  }
   ...
   * @param _upper(int) - The upper (integer) bound of the multiplicity interval.
   */
  protected boolean addCardinalityFact(PrimSig _ownerSig, String _fieldName, int _lower,
      int _upper) {
   ...
   */
  protected Set<Expr> addTransferFacts(PrimSig _ownerSig, Field _transfer, String _sourceFiledName,
      String _targetFieldName, List<Set<Field>> _sourceOutputsAndTargetInputsFields,
    ...
    if (_sourceOutputsAndTargetInputsFields != null) {
      if (_sourceOutputsAndTargetInputsFields.get(0).size() > 0) {
        Set<Expr> factsWithoutSig = AlloyExprFactory.exprs_transferInItems(_ownerSig, _transfer,
            Alloy.sources, _sourceOutputsAndTargetInputsFields.get(0));
      ...
      if (_sourceOutputsAndTargetInputsFields.get(1).size() > 0) {
        Set<Expr> factsWithoutSig = AlloyExprFactory.exprs_transferInItems(_ownerSig, _transfer,
            Alloy.targets, _sourceOutputsAndTargetInputsFields.get(1));
        facts.addAll(factsWithoutSig);
        if (!_toBeInherited)
          alloy.addToFacts(AlloyUtils.toSigAllFacts(_ownerSig, factsWithoutSig));
      }
    }
    alloy.addToFacts(AlloyExprFactory.exprs_bijectionFilteredFactsForSig(_ownerSig, _transfer,
   ...
   */
  protected Set<Expr> addTransferBeforeFacts(PrimSig _ownerSig, Field _transfer,
    ...
    Set<Expr> factsWithoutAll =
        addTransferFacts(_ownerSig, _transfer, _sourceFieldName, _targetFieldName,
            _sourceOutputsAndTargetInputsFields, _toBeInherited);
   ...
   */
  protected void addNoInputsOrOutputsFieldFact(PrimSig _ownerSig, String _fieldName,
      Func _inputsOrOutputs) {
   ...
   */
  protected void addInInputsAndNoInputXFacts(PrimSig _ownerSig, Set<Field> _fields,
      boolean _addEqual) {
   ...
   */
  protected void addInOutputsAndNoOutputsXFacts(PrimSig _ownerSig, Set<Field> _fields,
      boolean _addEqual) {
   ...
   */
  protected void addInOutClosureFact(PrimSig _ownerSig, Field _field, Set<Field> _fieldOfFields,
      Func _inputsOrOutputs) {
    List<Field> sortedfieldOfFields = AlloyUtils.sortFields(_fieldOfFields);
    alloy.addToFacts(
        AlloyExprFactory.expr_inOutClosure(_ownerSig, _field, sortedfieldOfFields,
            _inputsOrOutputs));
  }
   ...
   */
  protected void addNoTransferInXStepsFact(Set<Sig> _noTransferInXStepsFactSigs,
      Set<PrimSig> _leafSigs) {
    Object[] sigs =
        sigByName.values().stream().filter(sig -> !_noTransferInXStepsFactSigs.contains(sig))
            .toArray();
    for (Object sig : sigs) {
   ...
   */
  protected Set<Sig> addStepsFacts(Map<String, Set<String>> _stepPropertiesBySig,
      Set<PrimSig> _leafSigs) {
    ...
    for (String sigName : _stepPropertiesBySig.keySet()) {
      ...
      if (_leafSigs.contains(sig)) {
        if (_stepPropertiesBySig.get(sigName).size() > 0) { // {x.steps in ....} and {... in x.steps} for leaf signature
          alloy.addToFacts(
              AlloyExprFactory.exprs_stepsFields(sig, _stepPropertiesBySig.get(sigName), true,
                  true));
        } else {
   ...
   */
  protected void addRedefinedSubsettingAsFacts(PrimSig _ownerSig,
      Map<String, String> _propertyNameAndType) {

    for (String pName : _propertyNameAndType.keySet()) {
      alloy.addToFacts(AlloyExprFactory.expr_redefinedSubsetting(_ownerSig,
  ...
  protected void handleNoInputsOutputs(
    ...
    Set<String> inputFlowFieldTypes =
        _connectorsTargetInputPropertyNamesByClassName.size() == 0 ? null
    ...
    Set<String> outputFlowFieldTypes =
        _connectorsSourceOutputPrpertyNamesByClassName.size() == 0 ? null
    ...
    for (String sigName : _allSigNames) {
      ...
      boolean addEqual =
          _sigNameWithTransferConnectorWithSameInputOutputFieldType.contains(sigName) ? false
              : true;
      ...
      if (_connectorsTargetInputPropertyNamesByClassName.keySet().contains(sigName)) {
        ...
        // "{all x: B1 | x.vin in x.inputs} and {all x: B1 | x.inputs in x.vin} and {all x| no inputs.x}
        addInInputsAndNoInputXFacts(sig, inputsFields, addEqual);
      } else {
        if (inputFlowFieldTypes != null && inputFlowFieldTypes.contains(sigName))// Integer = type of what is flowing
      ...
      if (_connectorsSourceOutputPrpertyNamesByClassName.keySet().contains(sigName)) {
        Set<String> propertyNames = _connectorsSourceOutputPrpertyNamesByClassName.get(sigName); // [vout]
        ...
        // {all x: B1 | x.vin in x.outputs} and {all x: B1 | x.outputs in x.vin}" and "{all x| no outputs.x}"
